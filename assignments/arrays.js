...
let lastCar = 0;
const length = inventory.length - 1
const car = inventory[length]
...
let BMWAndAudi = [];
for (i = 0; i < inventory.length; i++){
    let carMake = inventory[i]
    if (carMake["car_make"] === "Audi" || carMake["car_make"] === "BMW")
    BMWAndAudi.push(carMake["car_model"])
}
